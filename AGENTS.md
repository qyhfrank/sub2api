...
2. 找到最新上游 tag（git tag -l 'v*' --sort=-v:refname | grep -E '^v[0-9]+\.[0-9]+\.[0-9]+$' | head -1）
3. 对比当前 HEAD 是否已包含该 tag（git merge-base --is-ancestor <tag> HEAD）
   - 已包含 -> 告知用户已是最新
   - 未包含 -> git merge <tag>，冲突则列出文件协助解决
4. 确定新 merge tag 序号：
   - git tag -l '<tag>.merge.*' --sort=-v:refname | head -1
   - 无已有 -> <tag>.merge.0，已有 N -> <tag>.merge.{N+1}
5. git tag <新 merge tag> && git push origin main --tags
6. 确认 Release workflow 已触发：gh run list --workflow=release.yml --limit 1
```
...
3. git tag <新 merge tag> && git push origin main --tags
```
