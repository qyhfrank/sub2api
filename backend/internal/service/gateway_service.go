...
func (s *GatewayService) isModelSupportedByAccount(account *Account, requestedModel string) bool {
	...
	if account.IsBedrock() {
		err := ValidateBedrockInvocationTarget(account, requestedModel)
		return err == nil
	}
...
func (s *GatewayService) forwardBedrock(
	...
	for {
		...
		// 错误/failover 处理
		if resp.StatusCode >= 400 {
			respBody, routeLocalQuota := inspectBedrockRouteLocalQuota429(resp)
			if routeLocalQuota {
				if target.Policy.Mode == "all_routes" && target.RouteKey != nil {
				...
				defer func() { _ = resp.Body.Close() }()
				return s.handleBedrockRouteLocalQuotaError(resp, respBody, c, account)
			}
...
func markBedrockRouteCooldown(account *Account, target BedrockInvocationTarget, blockedUntil int64) {
	...
	pool := runtimeBedrockRoutePools.getOrCreate(routePoolRegistryKey(account, target.Support.CanonicalModel, target.Policy), routes)
	pool.MarkCooldown(*target.RouteKey, blockedUntil)
}

func (s *GatewayService) handleBedrockRouteLocalQuotaError(
	resp *http.Response,
	respBody []byte,
	c *gin.Context,
	account *Account,
	...
	setOpsUpstreamError(c, http.StatusTooManyRequests, upstreamMsg, upstreamDetail)
	appendOpsUpstreamError(c, OpsUpstreamErrorEvent{
		Platform:           account.Platform,
		AccountID:          account.ID,
		AccountName:        account.Name,
	...
	if upstreamMsg == "" {
		return nil, fmt.Errorf("upstream error: %d", http.StatusTooManyRequests)
	}
	return nil, fmt.Errorf("upstream error: %d message=%s", http.StatusTooManyRequests, upstreamMsg)
}
