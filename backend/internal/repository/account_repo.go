...
func (r *accountRepository) SetModelRateLimit(ctx context.Context, id int64, scope string, resetAt time.Time, reason ...string) error {
	...
	rows, err := client.QueryContext(ctx, `
		SELECT extra->'model_rate_limits'->$1->>'rate_limit_reset_at'
		FROM accounts
		WHERE id = $2 AND deleted_at IS NULL
		FOR NO KEY UPDATE
	`, scope, id)
	...
	if existing.Valid {
		if existingResetAt, parseErr := time.Parse(time.RFC3339, strings.TrimSpace(existing.String)); parseErr == nil && !existingResetAt.Before(resetAt) {
			if tx != nil {
				if err := tx.Commit(); err != nil {
					return err
				}
